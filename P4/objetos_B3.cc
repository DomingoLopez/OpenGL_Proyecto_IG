...
_triangulos3D::_triangulos3D()
{
  _tipoMaterial material = MATERIAL1;
          ...
          //Componntes iniciales de los materiales
          ambiente_difusa._0 = 0;
          ambiente_difusa._1 = 0;
          ambiente_difusa._2 = 1;
...
void _triangulos3D::draw(_modo modo,_tipoMaterial tipoMaterial, float r1, float g1, float b1, float r2, float g2, float b2, float grosor)
{
switch (modo){
  ...
  case SOLID_ILUMINATED_FLAT: draw_iluminacion_plana(tipoMaterial);
  case SOLID_ILUMINATED_GOURAUD: draw_iluminacion_suave(tipoMaterial);
	}
}


/****************************************************************************
OBTENER NORMALES A LAS CARAS Para la Iluminación
*****************************************************************************/

void _triangulos3D::calcular_normales_caras(){

  //Damos tamaño al vector de las normales de las caras (el mismo que el número de caras)
  ...
  for(int i=0; i< caras.size();i++){
    ...
    //Éstas restas las podemos hacer así porque está sobrecargado el operador - del _vertex3f (Mirar en el vertex.h)
    //Si no, habría que restar la x con la x , y con y, z con z de ambos vértices.
    _vertex3f n=a1.cross_product(a2); //También está implementado el producto vectorial en el vertex.h

    //Realizamos el módulo para poder normalizarlo. Recordar: |n|² = x² + y² + z² (En vectores)
    float modulo =sqrt(n.x*n.x + n.y*n.y + n.z*n.z);
    //Normalizamos. Dividimos cada componente del vector por su módulo
    this->normales_caras[i] =  _vertex3f(n.x/modulo,n.y/modulo,n.z/modulo);

  }
  
 //ponemos el booleanos de normales caras como calculado
 this->b_normales_caras = true;
 //Con ésto hemos calculado todas las normales a las caras del objeto

}


/****************************************************************************
OBTENER NORMALES A LOS VÉRTICES Para la Iluminación
*****************************************************************************/

void _triangulos3D::calcular_normales_vertices(){


//ESTA COMPROBACIÓN ES SUPER NECESARIA, DADO QUE SI ACTIVAMOS ILUMINACIÓN SUAVE PUEDE QUE NO SE HAYAN CALCULADO LAS NORMALES A LAS CARAS. ES DECIR, PUEDE QUE AL ACTIVAR LA ILUMINACIÓN SUAVE DE PRIMERAS NO SE CARGAN LAS NORMALES A LAS CARAS Y AL CAMBIAR DE OBJETO DE UN CORE
if (normales_caras.size() != caras.size())
		calcular_normales_caras();

//Igual que para las caras, damos tamaño al vector de las normales a las caras
this->normales_vertices.resize(this->vertices.size());



//PRIMERA FORMA, un poco menos eficiente

_vertex3f aux; //vértice auxiliar (ó vector auxiliar) para calcular las normales de los vértices
aux.x=0;
aux.y=0;
aux.z=0;
...
//Recorro los vértices
  for(int i =0; i< vertices.size();i++){
    ...
    //Recorro todas las caras para cada vértice
    for(int j = 0; j< caras.size(); j++){

      if(caras[j]._0 == i || caras[j]._1 == i || caras[j]._2 == i){ //Si esta cara está pegando a ese punto

        aux.x+=normales_caras[j].x;
        aux.y+=normales_caras[j].y;
    ...
    //Una vez que para ese vértice ya hemos recorrido todas las caras y sumado las normales
    ...
    this->normales_vertices[i] = _vertex3f(aux.x/modulo, aux.y/modulo, aux.z/modulo);
    aux.x = 0;
    aux.y = 0;
    aux.z = 0;
  ...
  /*
  //SEGUNDA FORMA, MÁS EFICIENTE PERO PRESENTA PROBLEMAS: O(n) + O(n) = O(n)
  //Recorremos las caras directamente

  

  for (int i = 0; i < caras.size(); i++)
	{
    //Hacemos la suma de todas las normales a las caras de todos los vértices, recorriendo las caras
  ...
  //Finalmente ponemos el booleano de normales a los vértices a true

  this->b_normales_vertices = true;

  //Con ésto hemos calculado todas las normales a los vértices
...
void _triangulos3D::setMaterial(_tipoMaterial materialNuevo){


  if(this->material != materialNuevo){
      
      this->material = materialNuevo;

      switch(materialNuevo){
        case MATERIAL1:
          ambiente_difusa._0 = 0;
          ambiente_difusa._1 = 0;
          ambiente_difusa._2 = 1;
        ...
        case MATERIAL2:
          ambiente_difusa._0 = 0;
          ambiente_difusa._1 = 1;
          ambiente_difusa._2 = 0;
          ambiente_difusa._3 = 1;
...
void _triangulos3D::draw_iluminacion_plana(_tipoMaterial material){

  setMaterial(material);

if (!b_normales_caras) //si no se han calculado las normales a las caras, calcularlas
  calcular_normales_caras();
...
/*************************************
SACADO DE LA DOCUMENTACIÓN DE OPENGL : 

For the glMaterial, it's usual to set the Ambient and Diffuse colours to the natural colour of 
the object and to put the Specular colour to white. The emission colour is generally black for 
objects that do not shine by their own light.
...
for(int i=0; i<caras.size();i++){
  ...
  glNormal3fv((GLfloat *) &normales_caras[i]); //Se pasa una sola normal como "referencia"
  glVertex3fv((GLfloat *) &vertices[caras[i]._0]);
  glVertex3fv((GLfloat *) &vertices[caras[i]._1]);
  glVertex3fv((GLfloat *) &vertices[caras[i]._2]);
...
void _triangulos3D::draw_iluminacion_suave(_tipoMaterial material){

setMaterial(material);

if (!b_normales_vertices) //si no se han calculado las normales a los vertices, calcularlas
...
for(int i=0; i<caras.size();i++){
  ...
  glNormal3fv((GLfloat *) &normales_vertices[caras[i]._0]); //Para cada vértice se pasa su normal
  glVertex3fv((GLfloat *) &vertices[caras[i]._0]);
...
if(tapa == 1){
    ...
    for(int i=0; i<num-1;i++){
      cara_aux._0=k;cara_aux._1=vertices.size()-1;cara_aux._2=k+num_aux;
      caras.push_back(cara_aux);
      k+=num_aux;
    }

    //Añadimos la cara final 
    cara_aux._0=k;cara_aux._1=vertices.size()-1;cara_aux._2=0;
    caras.push_back(cara_aux);
...
void _chasis::draw(_modo modo, _tipoMaterial material, float r1, float g1, float b1, float r2, float g2, float b2, float grosor)
{
glPushMatrix();
glScalef(1.0,0.22,0.95);
base.draw(modo,material, r1, g1, b1, r2, g2, b2, grosor);
glPopMatrix();

glPushMatrix();
glRotatef(90.0,1,0,0);
rodamiento.draw(modo,material, r1, g1, b1, r2, g2, b2, grosor);
glPopMatrix();
...
void _torreta::draw(_modo modo, _tipoMaterial material, float r1, float g1, float b1, float r2, float g2, float b2, float grosor)
{
glPushMatrix();
...
parte_trasera.draw(modo,material, r1, g1, b1, r2, g2, b2, grosor);
glPopMatrix();
}
...
void _tubo::draw(_modo modo, _tipoMaterial material, float r1, float g1, float b1, float r2, float g2, float b2, float grosor)
{
...
glRotatef(90.0,0,0,1);
tubo_abierto.draw(modo,material, r1, g1, b1, r2, g2, b2, grosor);
glPopMatrix();
}
...
void _tanque::draw(_modo modo,_tipoMaterial material, float r1, float g1, float b1, float r2, float g2, float b2, float grosor)
{
glPushMatrix();
chasis.draw(modo,material, r1, g1, b1, r2, g2, b2, grosor);

glRotatef(giro_torreta,0,1,0);
glPushMatrix();
glTranslatef(0.0,(chasis.altura+torreta.altura)/2.0,0.0);
torreta.draw(modo,material, r1, g1, b1, r2, g2, b2, grosor);
glPopMatrix();

glPushMatrix();
glTranslatef(torreta.anchura/2.0,(chasis.altura+torreta.altura)/2.0,0.0);
glRotatef(giro_tubo,0,0,1);
tubo.draw(modo,material, r1, g1, b1, r2, g2, b2, grosor);
glPopMatrix();
glPopMatrix();
...
//Perfil de la esfera (Lo vamos a generar por ángulos.)
  ...
  int repeticiones = 20;//Estas repeticiones marcan el número de puntos del perfil
...
void _esfera::calcular_normales_vertices_exactos(){

  this->normales_vertices.resize(vertices.size());
  
  for(int i =0; i<vertices.size(); i++){

    normales_vertices[i] = _vertex3f(vertices[i].x/radio, vertices[i].y/radio, vertices[i].z/radio);
    
  }

  this->b_normales_vertices = true; //Hemos de ponerlo a true para evitar que haya cores, etc
...
caras.resize(12);
caras[0]._0=1;caras[0]._1=0;caras[0]._2=3;
...
caras[8]._0=2;caras[8]._1=7;caras[8]._2=6;
caras[9]._0=2;caras[9]._1=3;caras[9]._2=7;
...
_pinzaSpider::_pinzaSpider(float radio, float altura){
  ...
  caras[1]._0=1;caras[1]._1=7;caras[1]._2=2;
  caras[2]._0=2;caras[2]._1=7;caras[2]._2=6;
  caras[3]._0=2;caras[3]._1=6;caras[3]._2=3;
  caras[4]._0=3;caras[4]._1=6;caras[4]._2=5;
...
void _cuerpoCentral::draw(_modo modo,_tipoMaterial material, float r1, float g1, float b1, float r2, float g2, float b2, float grosor){


glPushMatrix();
glScalef(1,0.7,0.7);
glTranslatef(0,1,0);
centro.draw(modo,material, r1, g1, b1, r2, g2, b2, grosor);
glPopMatrix();
...
void _cuerpoTrasero::draw(_modo modo,_tipoMaterial material, float r1, float g1, float b1, float r2, float g2, float b2, float grosor){
...
trasero.draw(modo,material, r1, g1, b1, r2, g2, b2, grosor);
glPopMatrix();
...
void _cabezaSpider::draw(_modo modo,_tipoMaterial material, float r1, float g1, float b1, float r2, float g2, float b2, float grosor){
 
glPushMatrix();
glTranslatef(-0.55,0.7,0);
glRotatef(90,0,0,1);
glScalef(0.4,0.4,0.4);
cabeza.draw(modo,material, r1, g1, b1, r2, g2, b2, grosor);
glPopMatrix();
...
void _aguijonSpider::draw(_modo modo,_tipoMaterial material, float r1, float g1, float b1, float r2, float g2, float b2, float grosor){
 
glPushMatrix();
glTranslatef(1.5,0.7,0.0);
glRotatef(anguloActualZ,0,0,1);
glScalef(1.1,0.5,1.1);
aguijon.draw(modo,material, r1, g1, b1, r2, g2, b2, grosor);
glPopMatrix();
...
void _pinzasSpider::draw(_modo modo,_tipoMaterial material, float r1, float g1, float b1, float r2, float g2, float b2, float grosor){
  ...
  pinzaD.draw(modo,material,r1,g1,b1,r2,g2,b2,grosor);
  glPopMatrix();
  ...
  glRotatef(90,0,0,1);
  glScalef(0.2,0.5,0.2);
  pinzaI.draw(modo,material,r1,g1,b1,r2,g2,b2,grosor);
  glPopMatrix();
...
void _pataSpider::draw(_modo modo,_tipoMaterial material, float r1, float g1, float b1, float r2, float g2, float b2, float grosor){
...
salienteCuerpo.draw(modo,material, r1, g1, b1, r2, g2, b2, grosor);
glPopMatrix();
...
articulacion.draw(modo,material, r1, g1, b1, r2, g2, b2, grosor);
glPopMatrix();
...
glRotatef(-45,1,0,0);
glScalef(0.5,1,0.5);
intermedia.draw(modo,material, r1, g1, b1, r2, g2, b2, grosor);
glPopMatrix();
...
void _spider::draw(_modo modo,_tipoMaterial material, float r1, float g1, float b1, float r2, float g2, float b2, float grosor){

Scentro.draw(modo,material,r1,g1,b1,r2,g2,b2,grosor);
Strasero.draw(modo,material,r1,g1,b1,r2,g2,b2,grosor);
Scabeza.draw(modo,material,r1,g1,b1,r2,g2,b2,grosor);
Saguijon.draw(modo,material,r1,g1,b1,r2,g2,b2,grosor);
Spinzas.draw(modo,material,r1,g1,b1,r2,g2,b2,grosor);
for(int i=0; i<patas.size();i++){
  patas[i].draw(modo,material,r1,g1,b1,r2,g2,b2,grosor);
}
